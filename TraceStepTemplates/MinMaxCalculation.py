...
class MinMaxCalculation(NumpyBasedTraceStep):
    """
    ...
    def GetParameters(cls):
        ...
        """
        return [
            ParameterDefinition("Expected_Max", "IN", description=""),
            ParameterDefinition("Tolerance", "IN", description=""),
            ParameterDefinition("Expected_Diff_Max_Min", "IN", description=""),
        ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        Exp_Max = parameters['Expected_Max']
        Exp_Max_Min_Diff = parameters['Expected_Diff_Max_Min']
        Tolerance = parameters['Tolerance']
        for triggerRange in ranges:
            time = signal.GetTimestamps().tolist()
            signal_value = signal.GetValues().tolist()
            max_value = max(signal_value)
            min_value = min(signal_value)
            import matplotlib
            from matplotlib import pyplot
            pyplot.xlabel(time[signal_value.index(max_value)])
            pyplot.ylabel(max_value)
            #pass
