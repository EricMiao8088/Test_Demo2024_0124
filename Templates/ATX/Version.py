...
def GetDownloadLinkForATXMako(uploadSettings):
    ...
    try:
        ...
        clientVersion = u"{0}__v{1}".format(GetFullName().replace(" ", "_"), GetVersionString())
    except ImportError:
        clientVersion = u""

    return _GetDownloadLinkForATXMako(uploadSettings, clientVersion)


def _GetDownloadLinkForATXMako(uploadSettings, clientVersion):
    clientVersion = quote(clientVersion)  # Hash und andere Sonderzeichen ersetzen
    path = u"api/download-file/ATXGenerator?clientVersion={0}&authKey={1}" \
        .format(clientVersion, uploadSettings.authKey)
    if uploadSettings.contextPath:
        path = u"/".join((uploadSettings.contextPath, path))
