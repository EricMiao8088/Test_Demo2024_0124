...
class SettingsFromServerMode(object):
    ...
    def of(cls, useSettingsFromServer):
        for each in [SettingsFromServerMode.NEVER, SettingsFromServerMode.WHEREKEYWORD,
...
class Config(object):
    ...
    def LoadExternalSettings(settings, settingsMode, keyword=None):
        ...
        SPrint(u'useExternalSettings={0}, keyword={1}'.format(Config.__useExternalSettings,
                                                              Config.__keyword))
...
class Settings(object):
    """
    Provides the ATX generator settings.
    """
    ...
    def GetString(self, name):
        ...
        :rtype: str
        """
        return Config.GetSetting(self._reportapi, name)
    ...
    def GetBoolean(self, name):
        ...
        :rtype: bool
        """
        return Config.GetSetting(self._reportapi, name) == u'True'

    def GetList(self, name):
        """
        Read a list setting. List entries are separated by ';', e.g. SUCCESS;FAILED;ERROR
        ...
        :rtype: list[str]
        """
        value = Config.GetSetting(self._reportapi, name)
        return self.__ConvertValueLineToList(value)

    def __ConvertValueLineToList(self, spec):
        result = []
    ...
    def GetDict(self, name):
        """
        Read a dict setting. Dict entries are separated by ';', e.g. env=hil;priority=1
        ...
        """
        value = Config.GetSetting(self._reportapi, name)
        return self.__ConvertValueLineToDict(value)

    def __ConvertValueLineToDict(self, spec):
        result = {}
            ...
            for each in spec.split(u';'):
                ...
                if len(split) == 2:
                    ...
                    raise Exception(u"Could not parse setting '{}', use '=' to separate "
                                    u"key and value and ';' to separate key value pairs!".format(
                        spec))
