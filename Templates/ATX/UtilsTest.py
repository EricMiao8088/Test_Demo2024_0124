...
from .Utils import (CompareGlobalConstantsLists, GetExtendedWindowsPath,
                    ...
                    SplitVersionString, GetVerdictWeighting, ReplaceAsciiCtrlChars)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with zipfile.ZipFile(target) as zf:
                ...
                if sys.version_info > (3,):
                    ...
                    self.assertEqual(info.CRC, 258627456)
                    ...
                    self.assertEqual(info.file_size, 2890000)
                    self.assertEqual(info.filename, 'dummy.tmp')
                    self.assertEqual(info.flag_bits, 0)
                    self.assertEqual(info.header_offset, 0)
                    self.assertEqual(info.internal_attr, 0)
                    self.assertEqual(info.orig_filename, 'dummy.tmp')  # noqa
