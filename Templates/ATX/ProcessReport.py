...
from .ReviewUtils import ReviewCommentException

if sys.version_info < (3,):
    str = unicode


def ProcessReport(reportApi, isPackageExecution, uploadSettings):
    '''
    ...
    @return: Upload-Ergebnis bei erfolgreichem Upload, sonst leeres Dict.
    @rtype: dict
    '''
    ...
    try:
        ...
        hasSameVersion = serverVersion == GetVersion()
        ...
        warnMsgNotUpToDateVer = _(u"Der verwendete ATX-Reportgenerator v{0} ist "
                                  ...
                                  u".").format(GetVersion(), serverVersion,
                                               GetDownloadLinkForATXMako(uploadSettings),
        ...
        zipFileName = os.path.basename(reportApi.GetDbDir())
        ...
        testCaseRefs = worker.GetAtxTestCaseReferences()
        ...
        archiveFilesExcludeRecordings = \
            Config.GetSetting(reportApi, u'archiveFilesExcludeRecordings').strip() == u'True'

        excludeFiles = []
        if archiveFilesExcludeRecordings:
            excludeFiles.extend(worker.GetTraceFiles())

        additionalFilesZipPath = ScanReportDir(reportApi, Api(),
                                               dbDir,
                                               archiveMiscFiles, excludeFiles
        ...
        if additionalFilesZipPath:
            for eachRefPath in testCaseRefs:
                fileList.append({u'file': additionalFilesZipPath,
                                 u'ref': eachRefPath, u'comment': None,
        ...
        if Config.GetSetting(reportApi, u'uploadToServer') == u'True':
            ...
            uploader = UploadManager(reportApi, GetVersion(), u'file-upload',
                                     zipFileNameWithExtension, zipArchive.GetZipFilePath(),
                                     uploadSettings, u'json')

            success, infoDict = uploader.StartUpload()
            if success:
        ...
        EPrint(errorMessage)
    except ReviewCommentException as ex:
        EPrint(str(ex))
