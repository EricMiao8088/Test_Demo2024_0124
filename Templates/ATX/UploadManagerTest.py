...
class UploadManagerTest(unittest.TestCase):
    ...
    def testBigFileUpload(self):
        ...
        uploadSettings = UploadSettings(
            u'localhostInvalidUrl', 8085, True, u'test-guide', u'', None, 1)

        fileTemp = tempfile.NamedTemporaryFile(prefix=u"bigUploadFile", delete=False)
        try:
            # Beispieldatei mit > 1000 MB erzeugen
            ...
            try:
                ...
                self.assertTrue(not um.StartUpload()[0],
    ...
    def testUploadErrorLogOnUploadFailedOnMaintenanceStatus(self):
        ...
        try:
            ...
            # ASSERT
            self.assertFalse(um.StartUpload()[0])
    ...
    def testUploadToResourceAdapterBuffer(self):
        ...
        port = 44422
        capturedPath = []
        mock_server = HTTPServer(('localhost', port), resourceAdapterHandlerFactory(capturedPath))
        mock_server_thread = Thread(target=mock_server.serve_forever)
        mock_server_thread.daemon = True
        mock_server_thread.start()
        ...
        dbFile = os.path.join('TestReports', 'Package 0_2023-06-06_090142', 'Package 0.trf')
        when(reportApiMock).GetDbFile().thenReturn(dbFile)
        ...
        uploadSettings = UploadSettings(
            u'127.0.0.1', port, True, u'test-guide', u'', u'AuthKey==', 123)
        ...
        expectedQueryParam = urlencode({'reportName': 'Package 0'})

        self.assertTrue(expectedQueryParam in capturedPath[0])
...
def resourceAdapterHandlerFactory(capturedPath):
    class MockResourceAdapterRequestHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            capturedPath.append(self.path)

            self.send_response(202)
            self.end_headers()

    return MockResourceAdapterRequestHandler
