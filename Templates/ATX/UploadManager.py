...
class UploadManager(object):
    ...
    def __PostUpload(self, monitor, uploadTries):
        ...
        return requestService.PerformHealthCheckedRequest(self._GetBaseUrl(), self.__proxies,
                                                          DoPostUpload, GuiFeedback)

    def StartUpload(self):
        """
        Führt den zuvor präparierten Upload aus.
        :returns: Liefert True bei Erfolg, sonst False. Liefert außerdem die Upload-Informationen
        bei erfolgreichem Upload, sonst ein leeres Dict.
        ...
        if port > 0:
            ...
            trfPath = self.__reportApi.GetDbFile()
            reportName = os.path.splitext(os.path.basename(trfPath))[0]
            return self.__TransferThroughResourceAdapter(port, reportName), {}
    ...
    def __TransferThroughResourceAdapter(self, port, reportName):
        '''
        Transfers the file together with the upload information to the ResourceAdapter.
        :param: port: Port on which the ResourceAdapter accepts uploads
        :type: port: int
    ...
    def __SyncUpload(self, response, startUploadTime, serverLabel):
        ...
        :param serverLabel: Server, der ausgewählt wurde für die Log-Anzeige.
        :type serverLabel: str
        :returns: Liefert True, bei erfolgreichem Upload, sonst False. Liefert außerdem die
        Upload-Informationen bei erfolgreichem Upload, sonst ein leeres Dict.
        :rtype: tuple(bool, dict)
        """
