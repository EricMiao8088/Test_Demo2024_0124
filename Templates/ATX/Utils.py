...
from log import LEVEL_VERBOSE, DPrint, EPrint, WPrint
from lib.PathHandler import RelSubPath, IsSubpath
...
class FileToArchive(object):
    """
    Repräsentiert eine Datei, welche für das ATX-Archiv erfasst wird. Teilweise werden Dateien
    des Reports in einen temporären Unterordner des ATX-Reports kopiert. Daher wird sowohl ein Pfad
    als auch ein optionaler Originalpfad gehalten.
    """
    def __init__(self, path, origPath=None):
        """
        :type path: str
        :type origPath: str|None
        """
        self._path = path
    ...
    def GetRelPath(self, baseDir):
        """
        Gibt einen relativen Pfad zurück, falls sich die Datei im parentPath befindet.
        :type baseDir: str
        :return: Relativer Pfad oder None
        :rtype: str|None
        """
        # Zur Ermittlung des relativen Pfads, den ursprünglichen Ablageort verwenden
        winPath = GetExtendedWindowsPath(self._origPath or self._path)
        baseDir = GetExtendedWindowsPath(baseDir)
        relPath = None
        if IsSubpath(baseDir, winPath):
            relPath = os.path.dirname(RelSubPath(baseDir, winPath))
        return relPath
    ...
    def Exists(self):
        ...
        :rtype: bool
        """
        return os.path.exists(GetExtendedWindowsPath(self._path))
...
def IsDownstreamAnalysis(item):
    ...
    :rtype: boolean
    """
    if hasattr(item, u'IsDownstream') and callable(getattr(item, u'IsDownstream')):
        return getattr(item, u'IsDownstream')()
    return False
...
def CompareGlobalConstantsLists(a, b):
    ...
    for const in a:
        if FindDictInList(b, u'SHORT-NAME', const[u'SHORT-NAME']) == -1:
            return False

        if FindDictInList(b, u'VALUE', const[u'VALUE']) == -1:
            return False
    return True
...
def FindAssociatedFilesForTrace(filePath):
    ...
    :rtype: List[str]
    """
    extensions = ['.info', '.recscf']
    possibleFiles = [
        os.path.splitext(filePath)[0] + ext if ext == '.recscf' else filePath + ext
        ...
        with zipfile.ZipFile(target, u'w', zipfile.ZIP_DEFLATED, True) as zipHandler:
            if sys.version_info < (3,):
                ...
                for source in sources:
                    ...
                    zinfo.create_system = 0  # normalerweise "3" unter Linux
...
class ATXValidationError(Exception):
    """
