...
class ReviewTest(unittest.TestCase):
    ...
    def testGetXml(self):
        ...
        when(reportCommentMock1).GetAuthor().thenReturn(u'WerkOhneNamen')
        when(reportCommentMock1).GetOverriddenResult().thenReturn(u'SUCCESS')
        when(reportCommentMock1).GetTimestamp().thenReturn(1662107516)
        subject = Review(reportCommentMock1, "Level 1", 1, 1, None)
        subject.SetTestCaseRef('ref')
        subject.SetSummary('Das ist eine Summary!')
        subject.SetDefectClass('Fehlerklasse')

        # ACT
        actual = subject.GetXml()

        # ASSERT
        xml = ElementTree.tostring(actual, encoding='unicode', method='xml')
        self.assertEqual(
            u'<TESTCASE-REVIEW>'
            u'<VERDICT-REVALUATION>PASSED</VERDICT-REVALUATION>'
            u'<DEFECT>Fehlerklasse</DEFECT>'
            u'<SUMMARY>Das ist eine Summary!</SUMMARY>'
            u'<REVIEWER>WerkOhneNamen</REVIEWER>'
            u'<COMMENT>Level 1 -&gt; PASSED: Comment</COMMENT>'
            u'<DATE>2022-09-02T08:31:56+00:00</DATE>'
            u'<REF DEST="TEST-CASE">ref</REF>'
            u'</TESTCASE-REVIEW>', xml)
