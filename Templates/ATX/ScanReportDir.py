...
class ScanReportDir(object):
    ...
    def __init__(self, reportApi, api, scanDir, fileExp, filesToExclude=None):
        """
        Konstruktor.
        @param reportApi: Report API Objekt
        @type reportApi: ReportApi
        @param api: Zugriff auf die API, um an das aktuelle TestReport-Verzeichnis zu gelangen.
        ...
        @type fileExp: str
        @param filesToExclude: Dateien, die nicht in die Zip inkludiert werden sollen.
        @type filesToExclude: list[str]
        """
        self.__scanDir = scanDir
        self.__distinctFilesFound = set()  # Set, um Dateien nicht doppelt zu erfassen

        if not filesToExclude:
            filesToExclude = []

        canArchiveMiscFiles = self.__CanArchiveMiscFiles(reportApi, api)
        if not canArchiveMiscFiles or not fileExp:
        ...
        for searchExpression in fileExp.split(';'):
            discoveredFiles.extend(
                self.__ExecuteSearchExpression(scanDir, searchExpression))

        for discoveredFile in discoveredFiles:
            discoveredFileAbs = GetExtendedWindowsPath(os.path.join(scanDir, discoveredFile))
            if discoveredFileAbs not in filesToExclude and self.__IsNewAtxReport(scanDir,
                                                                                 discoveredFile):
                self.__distinctFilesFound.add(discoveredFile)
    ...
    def __CanArchiveMiscFiles(self, reportApi, api):
        ...
        if not archiveOnlyInTestRpeortFolder:
            return True
    ...
    def CreateZipArchive(self, zipFilePrefix, targetdir):
        ...
        @rtype: str oder None
        """
        if not self.__distinctFilesFound:
            return None
    ...
    def __IsNewAtxReport(self, dbDir, path):
        """
        ...
        return not checkHasMappingXml and not checkHasReportXml
