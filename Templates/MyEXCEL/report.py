...
def _ProcessContainerElement(reportApi, prjElem, prefix=''):
    ...
    for item in prjElem.IterItems():
        ...
        if itemType == 'Package':
            ...
            if item.IsSkipped():
                continue
            newPrefix = '%s%s' % (('%s%s' % (prefix, PACKAGE_NAME_SEPARATOR))
                                 if prefix else '', item.GetName())
...
def _ProcessPackage(item, prefix=None):
    ...
    packageName = '%s%s' % (('%s%s' % (prefix, PACKAGE_NAME_SEPARATOR)) if prefix
                                                                          else '', baseName)
    ...
    return (packageName, packageResult, packageDescription)

def __raiseExceptionShowDialogBefore(excMessage, excType=None, excTraceback=None):

    excMessage = 'Excel report generation failed, caused by:\n%s' % str(excMessage)

    if excType and excTraceback:
        raise excType(excMessage).with_traceback(excTraceback)
    else:
...
def _CreateTestCaseResultBorder(sheet, startCell, endCell):
    ...
    _CreateTableBorder(sheet, "{}:{}".format(startCell.coordinate, endCell.coordinate),
...
def _WritePkgDetails(workbook, reportApi, pkgRes):
    ...
    for i, (pkg, res, descr) in enumerate(pkgRes):
    ...
    if pkgRes and __GetSettingValue(reportApi, 'tableFrames'):
        try:
            ...
            raise RuntimeError('Error making Border: %s' % e)
...
def _WriteSummary(workbook, reportApi):
    ...
    for setting in REPORT_INFO_SETTINGS:
        ...
        try:
            ...
            raise RuntimeError('Setting parameter \'%s\' is incorrect! Error: %s'
                               % (setting, e))
...
def _GetWorkbook(reportApi, reportName):
    ...
    if not os.path.isdir(reportDir):
        raise RuntimeError('Report directory \'%s\' does not exist!' % reportDir)
    templateFile = reportApi.GetSetting('templateFile')
    if not templateFile:
        raise RuntimeError('No Excel template file referenced!')
    if not os.path.isabs(templateFile):
        curDir = os.path.dirname(__file__)
        templateFile = os.path.abspath(os.path.join(curDir, templateFile))
    if not os.path.isfile(templateFile):
        raise RuntimeError('Referenced Excel template \'%s\' does not exist!' % templateFile)
    ...
    ws = _EnsureSheetExists(wb, 'Overview')
    ws.add_image(Image(os.path.abspath(os.path.join(curDir, 'Logo.png'))), 'B2')
    ws = _EnsureSheetExists(wb, 'Configuration')
    ws.add_image(Image(os.path.abspath(os.path.join(curDir, 'Logo.png'))), 'B2')
...
def _GetConfigStartPosition(reportApi):
    ...
    try:
        ...
        raise RuntimeError('Setting parameter \'configEntriesStartAt\' is incorrect! '
                           'If defined, its value should be a tuple containing row and '
                           'column indexes.')
    return startcfgRow, cfgCol


def _WriteConstFiles(sheet, startRow, startCol, constFiles):
    _InsertText(sheet, 'Global constants definition files', startRow, startCol, font=Font(bold=True))
    writtenRows = 1

    if len(constFiles) > 0:
        for constFile in constFiles:
            _InsertText(sheet, '  %s' % constFile, startRow + writtenRows, startCol)
            writtenRows += 1
    else:
        _InsertText(sheet, '  No global constants definition files configured',
...
def _WriteBuses(sheet, startRow, startCol, buses):
    _InsertText(sheet, 'Bus access', startRow, startCol, font=Font(bold=True))
    writtenRows = 1
    if buses:
        ...
        for bus in buses:
            ...
            _InsertText(sheet, '  %s' % busId, startRow + writtenRows, startCol)
            _InsertKeyValue(sheet, '    Fibex channel', ch, startRow + writtenRows + 1, startCol)
            _InsertKeyValue(sheet, '    Bus database', db, startRow + writtenRows + 2, startCol)
            writtenRows += 3

    else:
        _InsertText(sheet, '  No Bus access configured', startRow + writtenRows, startCol)
        writtenRows += 1
...
def _WriteEcus(sheet, startRow, startCol, ecus):
    _InsertText(sheet, 'ECUs', startRow, startCol, font=Font(bold=True))
    writtenRows = 1
    if ecus:
        ...
        for ecu in ecus:
            ...
            _InsertText(sheet, '  %s' % ecuId, startRow + writtenRows, startCol)
            _InsertKeyValue(sheet, '    A2L file', a2l, startRow + writtenRows + 1, startCol)
            _InsertKeyValue(sheet, '    Hex file', hexFile, startRow + writtenRows + 2, startCol)
            _InsertKeyValue(sheet, '    SGBD', sgbd, startRow + writtenRows + 3, startCol)
            writtenRows += 4
    else:
        _InsertText(sheet, '  No ECUs configured', startRow + writtenRows, startCol)
        writtenRows += 1

    return writtenRows

def _WriteFius(sheet, startRow, startCol, fius):
    _InsertText(sheet, 'Failure simulation', startRow, startCol, font=Font(bold=True))
    writtenRows = 1
    if fius:
        ...
        for fiu in fius:
            ...
            _InsertText(sheet, '  %s' % FiuId, startRow + writtenRows, startCol)
            _InsertKeyValue(sheet, '    Fehlersimulations-Datei', path, startRow + writtenRows + 1, startCol)
            writtenRows += 2

    else:
        _InsertText(sheet, '  No FIUs configured', startRow + writtenRows, startCol)
        writtenRows += 1
...
def _WriteModels(sheet, startRow, startCol, models):
    _InsertText(sheet, 'Models', startRow, startCol, font=Font(bold=True))
    writtenRows = 1
    if models:
        ...
        for mdl in mdls:
            ...
            _InsertText(sheet, '  %s' % modelId, startRow + writtenRows, startCol)
            _InsertKeyValue(sheet, '    Model', path, startRow + writtenRows + 1, startCol)
            _InsertKeyValue(sheet, '    Us as time base', tb, startRow + writtenRows + 2 , startCol)
            writtenRows += 3
    else:
        _InsertText(sheet, '  No Models configured', startRow + writtenRows, startCol)
        writtenRows += 1
...
def _WriteCommonData(sheet, startRow, startCol, commonData):
    _InsertText(sheet, 'Common', startRow, startCol, font=Font(bold=True))

    _InsertKeyValue(sheet, '  Name of tester', commonData['editor'], startRow + 1, startCol)
    _InsertKeyValue(sheet, '  Name of test computer', commonData['testBench'], startRow + 2, startCol)
    _InsertKeyValue(sheet, '  Data directory', commonData['dataDir'], startRow + 3, startCol)
    _InsertKeyValue(sheet, '  Package directory', commonData['pkgDir'], startRow + 4, startCol)
...
def _WriteTbcDetails(sheet, tbcData, tbcPath, startRow, startCol):
    ...
    if tools:
        ...
        for tool in tools:
            ...
            status = statNames.get(tool['status'], 'unknown')

            _InsertText(sheet, toolId, startRow + writtenRows, startCol, font=Font(bold=True))
            _InsertKeyValue(sheet, '  Host', host, startRow + writtenRows + 1 , startCol)
            _InsertKeyValue(sheet, '  Status', status, startRow + writtenRows + 2 , startCol)
            _InsertKeyValue(sheet, '  Version', version, startRow + writtenRows + 3 , startCol)
            writtenRows += 4

    else:
        _InsertText(sheet, 'No tools configured', startRow, startCol)
        writtenRows += 1
...
def _WriteConfig(workbook, reportApi, config):
    ...
    if startRow is not None and startCol is not None:
        ...
        sheet = _EnsureSheetExists(workbook, 'Configuration')

        _InsertText(sheet, 'Test configuration', startRow, startCol,
        ...
        if tcfData:
            ...
            _InsertText(sheet, 'No test configuration loaded', startRow, startCol,
                        font=Font(bold=True))
            writtenRows += 1

        startRow += (writtenRows + 2)  # Leerzeilen
        _InsertText(sheet, 'Test-bench configuration', startRow, startCol,
                    font=Font(size=14, bold=True))
        writtenRows = 2  # Eine Leerzeile
        tbcData = config['tbc']
        if tbcData:
            ...
            _InsertText(sheet, 'No test-bench configuration loaded', startRow, startCol,
                        font=Font(bold=True))
            writtenRows += 1
...
def _CreateExecReportFromTemplate(reportApi, reportName, config, pkgRes, attrs):
    ...
    try:
        ...
        exc = sys.exc_info()
        msg = 'Writing the Excel report file failed, caused by:\n%s' % exc[1]
        __raiseExceptionShowDialogBefore(msg, exc[0], exc[2])
    finally:
